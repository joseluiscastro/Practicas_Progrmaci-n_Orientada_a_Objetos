...
class Materia{
    ...
    public function materiasAsignadas($maestro){
        echo"<div class='table-responsive sok_font'>";
        echo"<table class=table table-bordered>";
        $sql = "SELECT * FROM asig_maes WHERE Id_maes=$maestro";
        $consulta = mysql_query($sql)or die("Error de consulta mala 1");
        $cuantos = mysql_num_rows($consulta);
        for($i = 0; $i < $cuantos; $i++){
            $id_mat = mysql_result($consulta,$i,'Id_mate');
    ...
    public function asignarMateriaMaestro($maestro){
        ...
        for($i = 0; $i < $cuantos; $i++){
            ...
            $sql2 = "SELECT * FROM asig_maes WHERE Id_maes=$maestro AND Id_mate=$id_mat";
            $consulta2 = mysql_query($sql2)or die("Error de consulta 2");
